...
# size 25 and 10 respectively. 
#
# This neural network is tunable using a number of
# hyperparameters, namely: number of epochs, learning rate, regularisation term
# (L2) and momentum. 

# If run as main, this program will generate an output file
...
class NeuralNetwork:
    def __init__(self, layers, activation = 'tanh'):  
        ...
        for i in range(1, len(layers) - 1):  
            ...
            self.previous_delta.append((np.zeros((layers[i - 1] + 1, \
                layers[i] + 1))))  
    ...
    # function used to fit the nn to the training data. 
    # lmbda represents a L2 regularisation term        
    ...
    def preprocess_test_data(self, test_data):
        ...
        for test_inst in test_data:
            X_values = (np.array(np.fromstring(test_inst, dtype=float, \
                sep=",")))[1:].tolist()
            # divide training data values by 1000 as per suggestion by Justin
    ...
    def split_data(self, X, y, validation_data_size = 0.2):              
        ...
        X_train, X_test, y_train, y_test = train_test_split(X, y, \
            test_size = validation_data_size, random_state = 0)  
    ...
    def train(self, X_train, y_train, learning_rate = 0.001, \
        num_epochs = 400000, momentum = 0.0, lmbda = 0.1):
    ...
    def get_accuracy(self, X_test, y_test, show_confusion_matrix = False, \
        ...
        for i in range(X_test.shape[0]):  
            ...
            # take the index with the largest value
...
if __name__ == "__main__":
    ...
    split_data_dict = nn.split_data(preprocessed_data_dict['X'], \
    ...
    print nn.get_accuracy(split_data_dict['X_test'], split_data_dict['y_test'], \
        show_confusion_matrix, show_classification_report)

    print("Accuracy on validation data: ")
    print nn.get_accuracy(split_data_dict['X_train'], split_data_dict['y_train'], \
        show_confusion_matrix, show_classification_report)