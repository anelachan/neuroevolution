...
# mainly exists so not performing extra I/O on files
...
def get_accuracy(nn_hidden_structure,activation):
  ...
  return nn.get_accuracy(split_data_dict['X_test'], 
		split_data_dict['y_test'], show_confusion_matrix, show_classification_report)
...
class GANN(object):
	...
	def __init__(self, options=default_options):
		...
		for individual in range(0,self.options['population_size']):
			...
			new_individual = Individual( self.slice_vector(individual_arr), self.options['fitness_function'] )
	...
	# encoding in GA. hence it lives in the GANN class not the Individual class
	...
	def get_fitness_tuples(self,individuals): 
		fitness_scores = [ indiv.get_fitness() for indiv in individuals ] # WARNING: LIST HAS BEEN CHANGED IN-PLACE IN NN CLASS!
	...
	def generate_offspring(self, selected): 
		...
		num_to_generate = int(self.options['swap_rate'] * self.options['population_size'])
		...
		for iteration in range(0,num_to_generate):
			...
			if self.options['encoding'] == 'binary':
				...
				parent_2 = random.choice( selected ) # may get a repeat... having problem w/ set operations w/ integer vector
	...
	def evolve(self):
		...
		# get fitness tuples on offspring (kind of bending the evolution metaphor here)
		...
		self.fitness_tuples = (self.fitness_tuples[:(self.options['population_size']-num_to_replace)] 
...
def main():
	...
	nn.train(split_data_dict['X_train'], split_data_dict['y_train'], .001, 400000, 0, .1)
